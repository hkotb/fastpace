...
#include "fwd.hpp"
#include "../special_functions/beta.hpp" // for incomplete beta.
#include "complement.hpp" // complements
#include "detail/common_error_handling.hpp" // error checks
#include "detail/inv_discrete_quantile.hpp" // error checks
#include "../special_functions/fpclassify.hpp" // isnan.
#include "../tools/roots.hpp" // for root finding.
     ...
     namespace binomial_detail{
        ...
        inline bool check_dist_and_k(const char* function, const RealType& N, const RealType& p, RealType k, RealType* result, const Policy& pol)
        {
           if(check_dist(function, N, p, result, pol) == false)
              return false;
           if((k+1 < 0) || !(boost::math::isfinite)(k))
           {
               *result = policies::raise_domain_error<RealType>(
                  function,
                  "Number of Successes argument is %1%, but must be >= 0 !", k+1, pol);
               return false;
           }
           if(k+1 > N)
           {
               *result = policies::raise_domain_error<RealType>(
                  function,
                  "Number of Successes argument is %1%, but must be <= Number of Trials !", k+1, pol);
               return false;
           }
           return true;
        }
